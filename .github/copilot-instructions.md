...
### Java / Spring Boot Conventions ( src/api/)Backend 

- **Java 21**: Use records for DTOs, pattern matching, text blocks where appropriate
- **Spring Boot 3.4**: Minimal API style with `@RestController`. Use constructor injection (no field injection)
- **Records for DTOs**: Use Java records for request/response models. Use classes for mutable entities
- **REST controllers**: One controller per domain area. Use `@RequestMapping` for base path, `@GetMapping`/`@PostMapping` for methods
- **Service layer**: Business logic in `@Service` classes. Controllers delegate to services
- **Exception handling**: Use `@RestControllerAdvice` with `@ExceptionHandler` for global error handling. Return consistent error shapes
- **Jackson**: Use `@JsonProperty` for serialization. Records auto-serialize with Jackson
- **BCrypt**: Use Spring Security's `BCryptPasswordEncoder` for password hashing
- **JWT**: Use io.jsonwebtoken (jjwt) library. Tokens in HTTP-only cookies
- **Structured logging**: Use SLF4J with  no `System.out.println` in productionLogback 
- **Maven**: Use Spring Boot parent POM. Manage deps via `<dependencyManagement>`
- **Testing**: JUnit 5 + MockMvc for API tests. `@SpringBootTest` for integration tests

### Next.js / React Conventions ( src/web/)Frontend 

- **Server Components by default**: Only add `'use client'` when the component needs hooks, event handlers, or browser APIs
- **App Router**: Use `page.tsx`, `layout.tsx`  no `pages/` directoryconventions 
- **TypeScript strict mode**: No `any` types, no implicit returns
- **Tailwind CSS**: Utility classes  no custom CSS filesonly 
- **Component naming**: PascalCase for components, kebab-case for directories

### Test Framework Conventions

- **JUnit 5**: Use `@Test`, `@DisplayName`, `@Nested` for grouping. Assertions with `assertThat` (AssertJ) or JUnit assertions
- **MockMvc**: Use `mockMvc.perform(get("/api/..."))` for HTTP-level testing. No need for running server
- **Playwright**: `test.describe()` for grouping. Page Object Model. No hardcoded waits
- **Test isolation**: Each test must be  reset store between testsindependent 
