...
**Stack:** Spring Boot 3.4 (Java 21, Minimal API style) + Next.js (TypeScript, App Router) + Azure Cosmos DB
...
src/
 ...
 src/main/java/com/spec2cloud/api/   
 Application.java      # Main app class      
 config/               # Security, JWT config      
 model/                # Entities + DTOs (Java records)      
 controller/           # REST controllers      
 service/              # Business logic + data store      
...
|---|---|---|
| Unit + Integration (API) | `cd src/api && mvn test` | JUnit 5 + MockMvc |
| Frontend lint | `cd src/web && npm run lint` | ESLint |
| Playwright e2e | `npx playwright test --config=e2e/playwright.config.ts` | Against Aspire |
| Playwright smoke | `npx playwright test --grep @smoke` | Smoke tests only |
| All tests | `npm run test:all` | API + Frontend + Playwright |
...
| Target | Command |
|---|---|
| Backend | `cd src/api && mvn package -DskipTests` |
